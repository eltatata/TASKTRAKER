...
export default function TaskClient({ task }: { task: Task }) {
  ...
  const details = [
    ...
    {
      key: 'Created At',
      content: <p>{task.createdAt?.toLocaleTimeString()}</p>,
    },
    {
      key: 'Start Date',
      content: task.startDate ? (
        <p>{task.startDate?.toLocaleTimeString()}</p>
      ) : (
    ...
    },
    {
      ...
      content: task.endDate ? (
        <p>{task.endDate?.toLocaleTimeString()}</p>
      ) : (
        <p className="text-muted">not defined</p>
      ),
    },
